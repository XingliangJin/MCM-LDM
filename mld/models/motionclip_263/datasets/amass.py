...
from ..config import ROT_CONVENTION_TO_ROT_NUMBER
from .. import config
...
from mld.models.motionclip_263.utils.action_label_to_idx import action_label_to_idx, idx_to_action_label
...
class AMASS(Dataset):
    ...
    def __init__(self, datapath="data/amass/amass_30fps_legacy_db.pt", split="train", use_z=1, **kwargs):
        ...
        for seq_idx in range(n_sequences):
            n_sub_seq = (self.db['thetas'][seq_idx].shape[0]-1) // seq_len
            ...
            # feats = np.split(self.db['joints3d'][seq_idx][:n_frames_in_use], n_sub_seq)
            feats = np.split(self.db['feats'][seq_idx][:n_frames_in_use], n_sub_seq)
    ...
    def _load_feat(self, ind, frame_ix):
        feats = self._feats[ind][frame_ix]
        return feats
