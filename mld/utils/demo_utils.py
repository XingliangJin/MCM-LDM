...
# load example data for style text
def load_style_example_input(txt_path):
    file = open(txt_path, "r")
    Lines = file.readlines()
    count = 0
    texts, lens, style_texts = [], [], []
    # Strips the newline character
    for line in Lines:
        count += 1
        s = line.strip()
        ...
        lens.append(int(s_l))
        texts.append(s_t)
        style_texts.append(s_st)
        print("Length-{}: {} {}".format(s_l, s_t, s_st))
    return texts, lens, style_texts
